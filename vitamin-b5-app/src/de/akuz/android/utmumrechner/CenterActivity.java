...
public class CenterActivity extends MyAbstractActivity implements
		LocationListener {
	...
	protected void onResume() {
		...
		db.close();
		String dbCount = getResources().getQuantityString(
				R.plurals.you_have_in_db, list.size());
		textViewDatabaseCount.setText(String.format(dbCount, list.size()));
	}

	@Override
	public void onLocationChanged(Location location) {
		if (textViewAccuracy != null && textViewCoordinates != null && location != null) {
			textViewCoordinates.setText(CoordinateUtils
					.locationToMGRS(location));
			textViewAccuracy.setText(decimalFormat.format(location
					.getAccuracy()) + "m");
		}
	}
