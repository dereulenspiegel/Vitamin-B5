...
public class CSVWriter {
	...
	public CSVWriter(Writer writer) {
		this(writer, DEFAULT_SEPERATOR, DEFAULT_QUOTE_CHAR,
				DEFAULT_ESCAPE_CHAR, DEFAULT_LINE_END);
	}

	public CSVWriter(Writer writer, char seperator, char quotechar,
		...
		for (int i = 0; i < nextLine.length; i++) {
			...
			if (nextElement != null) {
				for (int j = 0; j < nextElement.length(); j++) {
					...
					} else if (escapechar != NO_ESCAPE_CHAR
							&& nextChar == escapechar) {
						sb.append(escapechar).append(nextChar);
					} else {
						sb.append(nextChar);
